...
add_task(async function test_highlights_renders_statements() {
  await withAITabDocument(
    async items => {
      ...
      highlights.heading = "A day on Niijima";
      ...
      Assert.equal(heading.textContent.trim(), "A day on Niijima", "Heading");
      const list = shadow.querySelector("ul");
      Assert.equal(
        list.getAttribute("aria-labelledby"),
        heading.id,
        "The list is named by the heading"
      );
      ...
      highlights.heading = "";
      await highlights.updateComplete;
      Assert.ok(!shadow.querySelector("h2"), "No heading element");
      Assert.ok(
