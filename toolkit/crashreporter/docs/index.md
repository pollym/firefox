...
: The minidump analyzer is a library that is compiled into the crash reporter
  client. It is used directly for main process crashes, or launched externally
  by the browser, to extract stack traces from the dump files generated during a
  crash. It appends the stack traces to the .extra file associated with the
  crash dump.

Crashping Crate

: The crashping crate is a library compiled into the crash reporter client. It
  is used directly for main process crashes, or launched externally by the
  browser, to deliver a crash ping to our telemetry servers.
...
The *crash reporter client* performs a number of roles. First, stack traces are
extracted from the dump via the *minidump analyzer* library.  The resulting
traces are appended to the .extra file of the crash together with the SHA256
hash of the minidump file. Once this is done a crash ping is assembled holding
the same information as the one generated by the `CrashManager` and it's
sent to the telemetry servers via the *crashping crate*.  Then, the *crash
reporter client* moves the dump data into the *pending* directory for the
configured data directory (defined via the `MOZ_CRASHREPORTER_DATA_DIRECTORY`
environment variable or from the UI). The crash reporter UI is platform
specific: there are separate versions for Windows, OS X, and Linux (using GTK).
The UI is a dialog giving the user the opportunity to submit this dump data to a
remote server.
...
Once the minidump file has been generated the `mozilla::dom::CrashReporterHost`
is notified of the crash. It will first try to extract the stack traces from the
minidump file using the *minidump analyzer* (invoked through the *crash reporter
client* binary). Then the stack traces will be stored in the extra file together
with the rest of the crash annotations and finally the crash will be recorded by
calling `CrashService.addCrash()`. This last step adds the crash to the
`CrashManager` database and automatically sends a crash ping with information
about the crash (again invoking the *crash reporter client* binary to submit the
ping).
...
Crash pings are considered telemetry, so they are opt-out (telemetry is on by
default). This differs from reports, which users explicitly send each time
(opt-in), unless they enable automatic submission of crash reports. Crash pings
contain a strict subset of the annotations in crash reports. Because they are
default on, they contain only annotations determined to be sufficiently
anonymous: there is no personal identifiable information. They do not include
the minidump, which contains detailed information about a crash and is sent with
reports.

Crash pings are always sent through the crash reporter client, using a unique
Glean instance that is not used for any other purpose. The Glean store is
persisted in the `Crash Reports` subdirectory of *UAppData*: it is not tied to a
single profile nor installation.

Firefox will perform maintenance tasks after startup that invoke the *crash
reporter client* to send pending crash pings, if any (and honor telemetry
...
- `MOZ_CRASHREPORTER_DISABLE` - Disable the crash reporting exception handler
