...
pub struct ContentClassifierEngineSizes {
    /// The Rust wrapper, the `Engine` stored inline in it, and the refcount
    /// box holding the filter data. Excludes the heap those point to, which
    /// the fields below account for.
    pub objects: usize,
