...
// Reports under ENGINES_PREFIX. The empty process name is the parent, which
// is the only place ContentClassifierService exists (it is MAIN_PROCESS_ONLY
// in components.conf).
async function collectReportsUnder(prefix) {
      ...
      if (!finished) {
        ...
        manager.getReports(
          (process, path, kind, units, amount) => {
            if (process === "" && path.startsWith(prefix)) {
...
async function collectEngineReports() {
  return collectReportsUnder(ENGINES_PREFIX);
}
...
add_task(async function test_live_engine_is_reported() {
  ...
  let reports = await collectEngineReports();
...
add_task(async function test_each_feature_reports_under_its_own_path() {
  ...
  let paths = (await collectEngineReports()).map(r => r.path);
...
async function engineLeafAmount(feature, leaf) {
  let reports = await collectEngineReports();
...
add_task(async function test_reported_sizes_track_list_size() {
  ...
  let smallRules = await engineLeafAmount("trackers", "filter-rules");
  ...
  let largeRules = await engineLeafAmount("trackers", "filter-rules");
  let largeHashes = await engineLeafAmount("trackers", "domain-hashes");
...
add_task(async function test_regex_table_is_reported_once_populated() {
  ...
  let table = await engineLeafAmount("trackers", "regex-table");
...
add_task(async function test_every_engine_leaf_is_reported() {
  ...
  Assert.greater(
    await engineLeafAmount("trackers", "resources"),
    0,
