...
public interface FirefoxAccountInterface {
     ...
     */
    fun `matchesServer`(`server`: FxaServer): kotlin.Boolean
     ...
     */
    fun `hasScope`(`scope`: kotlin.String): kotlin.Boolean
...
open class FirefoxAccount: Disposable, AutoCloseable, FirefoxAccountInterface
{
     ...
     * Check if an account was created from a config
     */
    @Throws(FxaException::class)override fun `matchesServer`(`server`: FxaServer): kotlin.Boolean {
    ...
    callWithHandle {
    uniffiRustCallWithError(FxaException) { _status ->
    UniffiLib.uniffi_fxa_client_fn_method_firefoxaccount_get_session_token(
        it,
        _status)
}
    }
     ...
     * # Arguments
     * - `scope` - space-separated list of OAuth scopes. Order is not significant.
     */override fun `hasScope`(`scope`: kotlin.String): kotlin.Boolean {
            return FfiConverterBoolean.lift(
    callWithHandle {
    uniffiRustCall() { _status ->
...
sealed class FxaEvent {
     ...
     */
    data class WebChannelPasswordChange(
        val `jsonPayload`: kotlin.String) : FxaEvent()
...
public object FfiConverterTypeFxaEvent : FfiConverterRustBuffer<FxaEvent>{
    override fun read(buf: ByteBuffer): FxaEvent {
        return when(buf.getInt()) {
            ...
            7 -> FxaEvent.WebChannelPasswordChange(
                FfiConverterString.read(buf),
                )
            8 -> FxaEvent.Disconnect
            9 -> FxaEvent.CallGetProfile
            else -> throw RuntimeException("invalid enum value, something is very wrong!!")
        }
    }
    ...
    override fun allocationSize(value: FxaEvent): ULong = when(value) {
        ...
        is FxaEvent.WebChannelPasswordChange -> {
            // Add the size for the Int that specifies the variant plus the size needed for all fields
            (
                4UL
                + FfiConverterString.allocationSize(value.`jsonPayload`)
            )
        }
    ...
    override fun write(value: FxaEvent, buf: ByteBuffer) {
        when(value) {
            ...
            is FxaEvent.WebChannelPasswordChange -> {
                buf.putInt(7)
                FfiConverterString.write(value.`jsonPayload`, buf)
                Unit
            }
            is FxaEvent.Disconnect -> {
                buf.putInt(8)
