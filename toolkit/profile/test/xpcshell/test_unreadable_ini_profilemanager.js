...
/*
 * Tests that --ProfileManager fails with NS_ERROR_PROFILE_DB_READ_FAILED when
...
add_task(async () => {
  ...
  try {
    selectStartupProfile(["--ProfileManager"]);
    Assert.ok(false, "Should have failed to select a profile.");
  } catch (e) {
    Assert.equal(
      e.result,
      Cr.NS_ERROR_NOT_AVAILABLE,
      "--ProfileManager should fail with NS_ERROR_NOT_AVAILABLE."
    );
  }
