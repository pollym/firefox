...
/*
 * Tests that -P fails with NS_ERROR_PROFILE_DB_READ_FAILED when profiles.ini
 * is unreadable.
 */
...
add_task(async () => {
  ...
  try {
    selectStartupProfile(["-P", DEDICATED_NAME]);
    Assert.ok(false, "Should have failed to select a profile.");
  } catch (e) {
    Assert.equal(
      e.result,
      Cr.NS_ERROR_NOT_AVAILABLE,
      "-P should fail with NS_ERROR_NOT_AVAILABLE."
    );
  }
