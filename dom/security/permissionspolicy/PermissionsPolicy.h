 ...
 * ~~~~~~~~~~~~~
 *
 * Each document and each HTMLIFrameElement have a PermissionsPolicy object
 * which is used to allow or deny features in their contexts.
 *
 * PermissionsPolicy is composed by a set of directives configured by the
 * 'Permissions-Policy' HTTP Header and the 'allow' attribute in
 * HTMLIFrameElements. Both header and attribute are parsed by the
 * PermissionsPolicyParser, which returns an array of Feature objects. Each
 * Feature object has a feature name and one of these policies:
 * - eNone - the feature is fully disabled.
 * - eAll - the feature is allowed.
 * - eAllowList - the feature is allowed for a list of origins.
 *
 * An interesting element of PermissionsPolicy is the inheritance: each context
 * inherits the permissions-policy directives from the parent context, if it
 * exists.
 ...
 *
 * PermissionsPolicy can be reset if the 'allow' or 'src' attributes
 * change in HTMLIFrameElements. 'src' attribute is important to compute
 * correcly the features via PermissionsPolicy 'src' keyword.
 *
 * When PermissionsPolicy must decide if feature X is allowed or denied
 * for the current origin, it checks if the parent context denied that feature.
...
class PermissionsPolicy final : public nsISupports, public nsWrapperCache {
 ...
 private:
  ...
  // This returns true if we have a declared permissions policy for
  // aFeatureName.
  ...
  // Permissions policy for the current context.
  nsTArray<Feature> mFeatures;

  // Declared string represents Permissions policy.
  nsString mDeclaredString;
