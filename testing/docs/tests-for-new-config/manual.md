...
1. Tests fail intermittently, we need a pattern to know if it is consistent or
   intermittent.
2. We still want to disable high frequency intermittent tests, those are just
   annoying.
3. You could be pushing off a bad base revision (regression or intermittent that
   comes from the base revision).
4. The machines you run on could be bad, skewing the data.
5. Infrastructure problems could cause jobs to fail at random places, repeated
   jobs filter that out.
6. Some failures could affect future tests in the same browser session or tasks.
7. If a crash occurs, or we timeout- it is possible that we will not run all of
...
8. Task failures that do not have a test name (leak on shutdown, crash on
...
Knowing the reasons for misleading data, here is a way to use [Push Health](https://treeherder.mozilla.org/push-health/push?revision=abaff26f8e084ac719bea0438dba741ace3cf5d8&repo=try&testGroup=pr).

- Alternatively, you could use the [API](https://treeherder.mozilla.org/api/project/try/push/health/?revision=abaff26f8e084ac719bea0438dba741ace3cf5d8)
...
The code sheriffs have been using [this documentation](https://wiki.mozilla.org/Auto-tools/Projects/Stockwell/disable-recommended)
